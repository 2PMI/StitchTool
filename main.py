...
class GUI(QMainWindow):
    def __init__(self):
        ...
        self.stitch_tool = StitchTool(flat='assets/flat/400/flat.tif',bg='assets/flat/400/bg.tif')
        self.load_flat()
    ...
    def stitch(self,no_show=False):
        ...
        # try:
        img_out = self.stitch_tool.stitch(src_path,save_path,
            grid_shape,
        ...
        if not no_show:
            self.single_show_window = SingleShowWindow(img=img_out, ui_path='./assets/single_show.ui')
    ...
    def stitch_batch(self):
        ...
        for target_dir in target_dirs:
            ...
            self.stitch_tool.set_flat(flat_info)
            try:
                self.stitch_tool.stitch(src_path,save_path,grid_shape,average_num=img_avg_num,overlay=float(self.line_overlay.text())/100.0,bias=bias,is_registration=self.reg_box.isChecked())
            except:
                msg_box = QMessageBox(QMessageBox.Warning, 'Warning', '发生未知错误，请检查参数是否正确！')
                msg_box.exec_()
                return
    ...
    def load_flat(self):
        ...
        self.flat_folder = {'None':{'flat':None,'bg':None}, 'Estimate':{'estimate'}}
