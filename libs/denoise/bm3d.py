    ...
    *_BlockPoint:当前block的坐标及大小
    ...
    for i in range(blk_num):
        for j in range(blk_num):
            tem_img = _noisyImg[
                present_x : present_x + Blk_Size, present_y : present_y + Blk_Size
            ]
            dct_Tem_img = cv2.dct(tem_img.astype(numpy.float64))
            m_Distance = numpy.linalg.norm((dct_img - dct_Tem_img)) ** 2 / (
                Blk_Size**2
            )
    ...
    *要将_similar_blocks第三维依次取出,然在频域用阈值滤波之后,再作反变换
    ...
    for i in range(m_Shape[1]):
        for j in range(m_Shape[2]):
            ...
            tem_Vct_Trans[numpy.abs(tem_Vct_Trans[:]) < Threshold_Hard3D] = 0.0
...
def Aggregation_hardthreshold(
    ...
    *对于最后的数组，乘以凯撒窗之后再输出
    ...
    if _nonzero_num < 1:
        _nonzero_num = 1
    block_wight = (1.0 / _nonzero_num) * Kaiser
    for i in range(Count):
        point = blk_positions[i, :]
        tem_img = (1.0 / _nonzero_num) * cv2.idct(_similar_blocks[i, :, :]) * Kaiser
        m_basic_img[
            point[0] : point[0] + _shape[1], point[1] : point[1] + _shape[2]
        ] += tem_img
        m_wight_img[
            point[0] : point[0] + _shape[1], point[1] : point[1] + _shape[2]
        ] += block_wight
    ...
    *返回的Wiener_wight用于后面Aggregation
    ...
    for i in range(m_Shape[1]):
        for j in range(m_Shape[2]):
            ...
            if m_weight != 0:
                Wiener_wight[i, j] = 1.0 / (m_weight**2 * sigma**2)
            # else:
...
@jit(nopython=True)
def Aggregation_Wiener(
    _Similar_Blks, _Wiener_wight, blk_positions, m_basic_img, m_wight_img, Count, Kaiser
):
    """
    *对3D变换及滤波后输出的stack进行加权累加,得到初步滤波的图片
    *_similar_blocks:相似的一组block,这里是频域的表示
...
def BM3D_2nd_step(_basicImg, _noisyImg):
    ...
    for i in range(int(Width_num + 2)):
        for j in range(int(Height_num + 2)):
            ...
            Similar_Blks, Similar_Imgs, Positions, Count = Step2_fast_match(
...
if __name__ == "__main__":
    cv2.setUseOptimized(
        True
    )  # OpenCV 中的很多函数都被优化过（使用 SSE2，AVX 等）。也包含一些没有被优化的代码。使用函数 cv2.setUseOptimized() 来开启优化。
