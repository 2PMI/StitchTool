...
class MergeTool(QMainWindow):
    def __init__(
        self,
        project_path=r"F:\PMJ\python\data\肠子实验\A1-新鲜样本\拼接2D/",
        ui_path="../../assets/channel_merge.ui",
    ):
    ...
    def load_channels(self):
        ...
        for channel_name in self.folders[folder]:
            if os.path.exists(
                os.path.join(self.root, channel_name, folder, "CellVideo/stitch.tif")
            ):
                channle_img = (
                    io.imread(
                        os.path.join(
                            self.root, channel_name, folder, "CellVideo/stitch.tif"
                        )
                    )
                    / 256.0
                )
    ...
    def merge(self):
        ...
        img_merge = np.zeros(
            (channle_1.shape[0], channle_1.shape[1], 3), dtype=np.uint8
        )
        del channle_1
        for i in range(3):
